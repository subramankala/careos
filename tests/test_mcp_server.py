...
def test_mcp_clinical_facts_read_tool_routes_to_internal_endpoint(monkeypatch) -> None:
    monkeypatch.setenv("CAREOS_MCP_API_KEY", "")
    ...
    response = mcp_server.call_tool(
        mcp_server.ToolCallRequest(
            tool="careos_get_clinical_facts",
            arguments={"tenant_id": "tenant-1", "patient_id": "p-123"},
        )
    )
    assert response.ok is True
    assert calls == [("/internal/patient-context/clinical-facts/active?tenant_id=tenant-1&patient_id=p-123", "GET", None)]


def test_mcp_observations_read_tool_routes_to_internal_endpoint(monkeypatch) -> None:
    monkeypatch.setenv("CAREOS_MCP_API_KEY", "")
